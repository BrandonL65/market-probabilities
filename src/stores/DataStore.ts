...
export class DataStore {
  ...
  */
  ...
  likelihoodOfComingBackToOpen = () => {
    let trades = {
      totalDays: 0,
      totalUpDays: 0,
      totalDownDays: 0,
      touchedLowerDeviation: 0,
      upDaysTouchedLowerDeviation: 0,
      downDaysTouchedLowerDeviation: 0,
      totalBounceBackToOpenAfterLowerDevTouch: 0,
      wentToUDafterOpen: 0,
      wentToLDafterOpen: 0,
    };
    for (let [k, candlesArr] of this.sorted1HData) {
      let dailyOpen = candlesArr[0].Open;
      let dailyClose = candlesArr[candlesArr.length - 1].Close;
      let upDay = dailyClose > dailyOpen ? true : false;
      let lowerDeviation = dailyOpen - 0.005;
      let upperDeviation = dailyOpen + 0.005;
      ...
      for (let i = 0; i < candlesArr.length; i++) {
        if (candlesArr[i].Low <= lowerDeviation) {
          trades.touchedLowerDeviation++;
          if (upDay) {
            trades.upDaysTouchedLowerDeviation++;
          } else if (!upDay) {
            trades.downDaysTouchedLowerDeviation++;
          }
          for (let j = i + 1; j < candlesArr.length; j++) {
            if (candlesArr[j].High >= dailyOpen) {
              trades.totalBounceBackToOpenAfterLowerDevTouch++;
              for (let k = j + 1; k < candlesArr.length; k++) {
                if (
                  candlesArr[k].Low <= lowerDeviation &&
                  candlesArr[k].High >= upperDeviation
                ) {
                  break;
                } else if (candlesArr[k].Low <= lowerDeviation) {
                  trades.wentToLDafterOpen++;
                  break;
                } else if (candlesArr[k].High >= upperDeviation) {
                  trades.wentToUDafterOpen++;
                  break;
                }
              }
              break;
